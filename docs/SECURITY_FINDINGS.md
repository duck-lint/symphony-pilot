...
| Aggregate task storage | **IMPLEMENTED / TARGETED-VERIFIED / REQUIRES STEP-8 REVIEW** | Activation blocker | Per-task containment ↔ host storage | Pilot now uses one shared fixed pool, explicit 64-GiB nominal versus 63-GiB allocatable capacity, global full-task reservations committed before helper mutation, capability-produced task binding/cleanup proof, and task-usable `f_bavail`/`f_favail` admission evidence. The current `/dev/sdd` root is rejected; dedicated quota-domain/helper provisioning and live task-binding proof remain open. | Step 8 | Required before unattended agent activation. | No; execution remains blocked. |

...
| SQLite documentation drift | **RESOLVED (Step 4)** | Documentation accuracy | Runtime capability ↔ Pilot scheduler configuration | Runtime implements the production SQLite tracker adapter. Pilot has not configured its managed scheduler to use it; scheduler cutover remains Step 5. Conflating those facts obscures the actual authority boundary. | Step 4 | None; correction does not perform cutover. | No. |

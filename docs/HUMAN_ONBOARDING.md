...
The generic Symphony lifecycle, sequential project-manager/planner/implementer/reviewer/adversary/archivist issue loop, workspace recovery, workpad mechanics, secret isolation, retry handling, publication preflight, deployment mechanics, and process management already belong to `symphony-pilot`. Do not ask the human to redesign or manually reproduce them.

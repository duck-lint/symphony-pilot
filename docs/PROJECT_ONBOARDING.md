...
Create a profile with repository, Git remote, secret reference, dispatch and blocked labels, execution limits, Codex settings, toolchain hint, and optional notification/sleep preferences. Do not add host paths or service names: deployment, workspace, state, logs, credentials, process state, locks, workflow location, service identity, and dashboard port are derived from the slug.

 ...
 * Q_X{GET,SET}QUOTA/PRJQUOTA for kernel limits.  It temporarily tightens one
 * limit at a time and performs a bounded write/create probe, restoring the
 * reviewed limits before emitting evidence.  A failed restore is fatal.
 */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
...
static int open_task(const char *project, const char *identifier,
                     uint32_t project_id, int create, char *path, size_t path_size) {
    int root = -1, project_fd = -1, task_fd = -1;
    struct stat st;
    ...
    if (attrs.fsx_projid == 0) {
        ...
        if (ioctl(task_fd, FS_IOC_FSGETXATTR, &attrs) || attrs.fsx_projid != project_id)
            goto fail;
    }
    close(project_fd); close(root);
    return task_fd;
fail:
...
static int quota_get(uint32_t id, struct dqblk *quota) {
    return quotactl(QCMD(Q_XGETQUOTA, PRJQUOTA), POOL_ROOT, (qid_t)id,
                    (char *)quota);
}

static int quota_set(uint32_t id, uint64_t byte_limit, uint64_t inode_limit) {
    struct dqblk quota;
    ...
    return quotactl(QCMD(Q_XSETQLIM, PRJQUOTA), POOL_ROOT, (qid_t)id,
                    (char *)&quota);
}

static int restore_limits(uint32_t id, uint64_t bytes, uint64_t inodes) {
    return quota_set(id, bytes, inodes);
}

static int byte_probe(int task_fd, uint32_t id, struct dqblk *before) {
    char name[] = ".symphony-byte-probe";
    int fd = -1, ok = 0;
    uint64_t temporary = before->dqb_curspace / QUOTA_BLOCK;
    if (temporary == 0) temporary = 1;
    if (quota_set(id, temporary * QUOTA_BLOCK, before->dqb_curinodes + 1)) return 0;
    ...
    if (restore_limits(id, before->dqb_bhardlimit * QUOTA_BLOCK,
                       before->dqb_ihardlimit)) return 0;
    return ok;
}

static int inode_probe(int task_fd, uint32_t id, struct dqblk *before) {
    char name[] = ".symphony-inode-probe";
    int fd = -1, ok = 0;
    uint64_t temporary = before->dqb_curinodes;
    if (temporary == 0) temporary = 1;
    if (quota_set(id, (before->dqb_curspace / QUOTA_BLOCK + 1) * QUOTA_BLOCK,
                  temporary)) return 0;
    fd = openat(task_fd, name, O_CREAT | O_EXCL | O_WRONLY | O_NOFOLLOW | O_CLOEXEC, 0600);
...
static int admit(const char *project, const char *identifier,
    ...
    if (geteuid() != 0 || !parse_identity(project, identifier, &id) ||
        ...
        return 2;
    task_fd = open_task(project, identifier, id, 1, path, sizeof(path));
    if (task_fd < 0 || quota_set(id, byte_limit, inode_limit) || quota_get(id, &quota)) return 3;
    if (quota.dqb_bhardlimit != (byte_limit / QUOTA_BLOCK) ||
        quota.dqb_ihardlimit != inode_limit) { close(task_fd); return 4; }
    struct fsxattr attrs;
    if (ioctl(task_fd, FS_IOC_FSGETXATTR, &attrs) || attrs.fsx_projid != id ||
        !byte_probe(task_fd, id, &quota) || quota_get(id, &quota) ||
        !inode_probe(task_fd, id, &quota) || quota_get(id, &quota)) { close(task_fd); return 5; }
    printf("{\"schema\":\"symphony-pilot-task-quota-proof/v1\",\"identifier\":\"%s\",\"workspace_path\":\"%s\",\"project_id\":%u,\"workspace_project_id\":%u,\"byte_hard_limit\":%llu,\"inode_hard_limit\":%llu,\"usage\":{\"bytes\":%llu,\"inodes\":%llu},\"byte_probe\":{\"attempted\":true,\"result\":\"EDQUOT\"},\"inode_probe\":{\"attempted\":true,\"result\":\"EDQUOT\"}}\n",
...
static int release_task(const char *project, const char *identifier) {
    ...
    if (errno != ENOENT || quota_get(id, &quota) || quota.dqb_curspace || quota.dqb_curinodes ||
        quota_set(id, 0, 0) || quota_get(id, &quota) || quota.dqb_curspace || quota.dqb_curinodes) goto fail;
...
int main(int argc, char **argv) {
    ...
    if (!operation || !project || !identifier) return 2;
